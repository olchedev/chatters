/// Contains App default dimensions
class AppDimensions {
  const AppDimensions._();

  /// t size, contains [tiny] interval sizes
  static const xt = 2.0;

  /// s size, contains [small] interval sizes
  static const s = 8.0;
  static const xs = 10.0;
  static const xxs = 15.0;

  /// m size, contains [medium] interval sizes
  static const m = 20.0;
  static const xm = 25.0;
  static const xxm = 30.0;
  static const xxxm = 36.0;

  /// l size, contains [large] interval sizes
  static const l = 50.0;

  /// h size, contains [huge] interval sizes
  static const h = 100.0;

  static const showHidePasswordIconSize = AppDimensions.xxm;

  static const snackBarRadius = AppDimensions.xs;

  static const userIconImagePadding = AppDimensions.xt;

  static const inputTextFieldLabelSize = AppDimensions.xxs;
  static const inputTextFieldLabelPadding = AppDimensions.xs;
  static const inputTextFieldVerticalMargin = AppDimensions.s;
  static const inputTextFieldHorizontalMargin = AppDimensions.m;
  static const inputTextFieldBorderRadius = AppDimensions.xs;

  static const elevatedButtonRadius = AppDimensions.xxxm;
  static const elevatedButtonNameVerticalPadding = AppDimensions.xxs;
  static const elevatedButtonNameHorizontalPadding = AppDimensions.xxxm;
  static const elevatedButtonNameSize = AppDimensions.xxs;

  static const outlinedButtonRadius = AppDimensions.xxxm;
  static const outlinedButtonNameVerticalPadding = AppDimensions.xxs;
  static const outlinedButtonNameHorizontalPadding = AppDimensions.xxxm;
  static const outlinedButtonNameSize = AppDimensions.xxs;

  static const headline1LetterSpacing = AppDimensions.xt;
  static const headline1Size = AppDimensions.xxxm;
}
